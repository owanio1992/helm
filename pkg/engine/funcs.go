...
func unzstdFunc(str string) (string, error) {
	...
	// 10MB limit for decompressed content
	const maxOutputLimit = 10 * 1048576
	...
	limitR := io.LimitReader(reader, maxOutputLimit+1)
	b, err := io.ReadAll(limitR)
	if err != nil {
		return "", err
	}

	if len(b) > maxOutputLimit {
		return "", fmt.Errorf("unzstd: decompressed content exceeds size limit of %d bytes", maxOutputLimit)
	}
...
func unlz4Func(str string) (string, error) {
	...
	if len(b) > maxOutputLimit {
		return "", fmt.Errorf("unlz4: decompressed content exceeds size limit of %d bytes", maxOutputLimit)
	}
...
func unsnappyFunc(str string) (string, error) {
	...
	const maxOutputLimit = 10 * 1048576
	if decodedLen > maxOutputLimit {
		return "", fmt.Errorf("unsnappy: decompressed content exceeds size limit of %d bytes", maxOutputLimit)
	}
...
func unbrotliFunc(str string) (string, error) {
	...
	if len(b) > maxOutputLimit {
		return "", fmt.Errorf("unbrotli: decompressed content exceeds size limit of %d bytes", maxOutputLimit)
	}
